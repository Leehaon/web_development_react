...
useContext(Context)를 통해 Context를 호출하고 컴텍스트 내의 변수 state, dispatch를 사용할 수 있도록 선언.
만약 컨텍스트를 통해 받아온 state.Login이 ture라면 로그인이 된 상태이므로 welcome user를 렌더링하고, false라면 LoginForm을 실행시키도록 작성(App.jsx의 삼항연산자 부분).
...
그런데 이 이틀 이전에도 한 번 로그인 폼을 작성 한적이 있었는데
그때
...
useMemo에서의 Memo는 memoiztion을 의미합니다.
...
두 번째 argument의 의존성 배열에 element가 들어갈 때는 의존성 배열의 return값이 변경될 때만 콜백 함수가 실행되는데, 이 때 렌더링 과정에서 배열 안의 element인 itme 값이 변경되었는지를 확인하고, 값이 변경된 경우에만 콜백 함수를 동작시켜서 메모이제이션된 값만 다시 계산합니다.
...
2. useMemo는 '렌더링 과정 중에 실행'됩니다. 렌데링 과정 중에 의존성 배열의 값이 변경되었는제를 확인하고, 값이 변경되었다면 이전에 저장한 값과 비교하여 값이 다른 경우에만 리렌더링 해줍니다. 따라서 useMemo는 메모이제이션 기법을 통해 불필요한 연산을 최소화하고 리렌더링을 막을 수 있으므로 렌더링 성능을 최적화할 때 사용합니다.
...
function App() {
  ...
  // number 값을 바꾸는 함수
  const increaseNum = () => {
    setNumber((prev) => prev + 1);  // 이 setNumber(number + 1);이 아니었는지 생각하기
  }
  ...
  return (
    <>
      ...
      <br />
      <h4>✨ 문자 바꾸기</h4>
      <input type="text" onClick={handleText} />
      <br />
      <br />
