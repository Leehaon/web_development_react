...
pageNotFound.tsx를 만드시고, return h1 사이즈의 Page Not Found를 작성하세요.
...
항상 stackoverflow 및 구글링을 통해 정보를 수집하는 후에 작성할 필요가 있습니다.
...
App.jsx의 Header 부분을 분리하여 컴포넌트화하세요.
...
2. 배열의 특징 element를 뽑아오는 방법 -> index 넘버를 사용 0, 1, 2, ... 순서
...
Fundamental, Crucial, Core 셋 중에 하나의 element가 임의적으로 출력될 수 있도록
...
getRandomInt(2)를 호출했을 경우 -> 0, 1, 2 셋 중의 하나에 해당하는 함수가 반환되기 때문에.
...
```
로 작성하게 되면 페이지 새로고침을 할 때 마다 배열 내의 랜덤한 element를 출력하게 함.
